...
public class tmp{// where k represent the vehicle number
	static double RC_EPS = 1.0e-6;

	public static void main(String[] args) throws IloException, IOException {
		DecimalFormat df = new DecimalFormat("0.00");
		// ---read data ---//
		String filePathFac = "/Users/jessicanini/Desktop/TVRP/auto-gen/data_01.txt";
		...
		int[][] Demand = new int[N][Nt+1];//TODO
		...
		for (int i = 0; i < N; i++) {
			...
			for (int j = 1; j < Nt+1; j++) {
				Demand[index][j] = Integer.parseInt(temp3[3 + j]);//TODO
			}
			for (int k = 0; k < Ng; k++) {
		...
		for (int i = 0; i <= Nd * up_l * C_large - 1; i++) {  //TODO
			LV.add(i);
		}
		...
		// calculate distance cost Nd/Ns Nd/Nc Ns/Nc Nc/Nc
		for (Integer i : depot) {
...
//TODO   SV LV  instead of K
		...
		for (int i = 1; i < Nd + Ns + Nc; i++) { //TODO  0->1
			for (int j = 1; j < Nd + Ns + Nc; j++) {
				for (int k : SV) {
					for (int t = 1; t < T; t++) {
						x[i][j][k][t] = cplex.intVar(0, 1, "x" + i + "," + j + "," + k+","+t);//TODO
						variable_count += 1;
		...
		for (Integer i : s_c) {//TODO
			for (Integer k : SV) {//TODO
				for (int t = 1; t < T; t++) {  // 0->1
		...
		for (Integer i : satellite) {//TODO
			for (Integer k : LV) {
				for (int t = 1; t < T; t++) {  // 0->1
					u[i][k][t] = cplex.intVar(0, 1, "u" + i + "," + k + "," + t);
					//System.err.println(u[i][k][t]);
					variable_count += 1;
					if (k % up_l == 0) {
		...
		for (int i = 0; i < N; i++) {
			for (Integer g : Gset) {
				for (int t = 1; t < T; t++) {  //0->1
					...
					//System.err.println(demand[i][g][t]);
					variable_count += 1;
				}
			}
		}
		...
		cplex.addMinimize(cplex.sum(exprObj1, exprObj2, exprObj3, exprObj4));  //TODO

		// System.out.println("Start constraint");
		for (Integer i : s_c) { // TODO
			for (Integer g : Gset) {
				IloLinearNumExpr constraint0 = cplex.linearNumExpr();
				constraint0.addTerm(1, inventory[i][g][0]);
				System.err.println(constraint0);
				cplex.addEq(constraint0, initial_inventory[i][g]);
				st_count += 1;
			}
		}
		// --- constraint1 --- //
		for (Integer i : satellite) {
			for (Integer g : Gset_0) {
				for (Integer t : Tset) {
					...
					for (int k = (i-Nd) * up_s * C; k <= (i-Nd) * up_s * C + up_s * C - 1; k++) {// sv_i
						for (Integer j : customer) {
		...
		for (Integer i : customer) {
			for (Integer t : Tset) {
				...
				for (Integer g : Gset) {
					...
					//System.err.println(" i "+i+" g "+g+" t "+t+" ");
				}
				//System.err.println(constraint6);
				//System.err.println(Demand[i][t]);//TODO
		...
		for (Integer i : satellite) {
			for (Integer t : Tset) {
				...
				//System.err.println(constraint7);//TODO
		...
		for (Integer t : Tset) {
			for (Integer i : satellite) {
				for (Integer c : Cvehicle) {
					...
					for (int k = (i-Nd) * up_s*C + c*up_s; k < (i-Nd) * up_s*C + (c+1)*up_s; k++) {//TODO sv_i_c
				...
				for (Integer t : Tset) {
					for (Integer k : SV) {
						if (k % up_s != 0) {
					        ...
					        double f1 = new BigDecimal(distance[j][i]).setScale(0, BigDecimal.ROUND_HALF_UP).doubleValue();
							cplex.addLe(constraint29_1, f1 + M);
							cplex.addGe(constraint29_2, f1 - M);
							st_count += 2;
						}
					}
				}
			...
			for (Integer j : s_c) {
				for (Integer t : Tset) {
					for (Integer k : SV) {
				        ...
				        double f1 = new BigDecimal(distance[j][i]).setScale(0, BigDecimal.ROUND_HALF_UP).doubleValue();
						cplex.addLe(constraint30_1, f1 + M);
						cplex.addGe(constraint30_2, f1 - M);
						st_count += 2;}
					}
				}
			}
        ...
        cplex.exportModel("/Users/jessicanini/Desktop/TVRP/tvrp1.lp");
