...
/*      编码方式 为  Depot 为0
 * 		1...numSat 为 卫星的点，  
 * 		numSat+1...numSat+numCus  为客户的点。    Demand 为客户需求。
 * 
 */
...
//        this.fitness=1/(totalDistance+0.00001);  
    ...
    public int getSatStart(){
    	...
    	while (i < arrNode.size()) {
    		...
    		if (index <= numSat){
    			if (!checkSat[index]) checkSat[index] = true;
    			else {
    				break ;
    			}
    		}
    ...
    public void calTotalDistance() {
        ...
        double[] capDem = new double[numSat+2];   // capDem   应该记录的是 同一个卫星的所有 路径中出现的最大正值的和  需要修改
        carCount = 0; truckCount =0;
        truckEnd.clear();
		double currentDemand = 0;
		int start = 0; 
		int index, satStart = getSatStart();  // 得到检测是否是第一个作为起点的 SAT位置
        ArrayList<Node> tempArrNode1 = new ArrayList<Node>(),tempArrNode2 = new ArrayList<Node>();
        
        // 先算小车 再算大车  这样可以知道小车点的需求
        ...
        while (from<arrNode.size()){
        	...
        	// 如果是sat  如果到了小车路径了
        	index = Integer.parseInt(arrNode.get(to).index);
        	// 如果这个sat 后面没有被使用到，或者 不需要进货。  点在这里留着，但是不计算路径。
...
//        	else {
        	...
        	tempDist += calDistNode(arrNode.get(from), arrNode.get(to));
        	if (index > numSat)    currentDemand += arrNode.get(to).demand;
        	else if (index > 0)    currentDemand += capDem[index];
        	tempTotalDist=tempTotalDist+tempDist;
        	tempArrNode1.add(arrNode.get(to));
        	from++;
        }
    ...
    public void createGen(){
        ...
        for (int i=0; i<numCus; i++) randCusIndex.add(numSat+i+1);
        for (int i=0; i<numSat; i++) randSatIndex.add(i+1);
...
//				if (currentDemand[start] + nodeList.get(toCus).demand > this.truckDemand || 
//	    			    currentDemand[start] + nodeList.get(toCus).demand < -this.truckDemand){  
...
//    public void createRoute(int start, double[] currentDemand, double capacity){
//    	Random rand = new Random();
//    	// 终止
