...
public class ColumnGen {
	...
	public class SubProblem {
		...
		public void createModel() {
			try {
				...
				// define parameters
				double M = 0;
				for (int i=2; i<=all_customers.size(); i++) 
					for (int j=2; j<=all_customers.size(); j++) {
	...
	private void ReadData() {
		File file = new File("C:/learning part/operationResearchh/code/learning/CG-data/R101.txt");
        ...
        try {
            ...
            while ((tempString = reader.readLine()) != null) {
                String[] thisString = tempString.split("\\s+");
                double[] thisDouble = new double[thisString.length];
                for (int i = 1; i < thisString.length; i++){
                	thisDouble[i-1] = Double.valueOf(thisString[i]);
                }
                if((int)thisDouble[0] == 1) {
                	// Depot
                	int idEx = (int)thisDouble[0];
                	double xCoord = thisDouble[1];
                	double yCoord = thisDouble[2];
                	depot_start=new Depot(idEx,xCoord,yCoord);
                	depot_end=new Depot(idEx,xCoord,yCoord);
                }
                else {
                	// customer
                	int idEx = (int)thisDouble[0];
                	double xCoord = thisDouble[1];
                	double yCoord = thisDouble[2];
                	double demand = thisDouble[3];
                	double readyTime = thisDouble[4];
                	double dueTime = thisDouble[5];
                	double serviceTime = thisDouble[6];
                	new Customer(idEx,xCoord,yCoord,demand,readyTime,dueTime,serviceTime);
                }
                line++;
            }
            reader.close();
